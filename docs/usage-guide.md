...
`dbgvis print EXPR` takes the same `-m` (`--mode`), `-b` (`--buffer`), and `-a` (`--alternate`) options as GDB, and `dbgvis types` lists the registered entries with their capabilities. `dbgvis print --all` (or `dbgvis all`) renders every in-scope local whose type is registered — one `name = value` per line — and lists the locals it skipped with the reason (unregistered, or ambiguous to LLDB). Its output is paged through `$PAGER` (default `less -R -F -X`) so a large frame does not scroll off; add `--no-pager`, or it falls back to inline output when there is no terminal. The expression is read through LLDB's `GetValueForVariablePath`, which resolves `foo`, `foo.bar`, and `foo[0]` straight from DWARF; LLDB's own Rust expression evaluator is unreliable and is only a fallback.
