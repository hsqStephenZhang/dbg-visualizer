...
def _toolchain_mismatch(actual, built_for):
    """The project's build toolchain differs from the one the driver was built for.

    A rustc_driver binary is ABI-bound to the compiler and sysroot it was compiled
    against, so the project must build with that same toolchain. Either rebuild the
    driver for the current toolchain, or build with the driver's toolchain.
    """
    date = built_for.rsplit(" ", 1)[-1].rstrip(")")
    toolchain = f"nightly-{date}"
    return (f"dbgvis auto: this build uses a different toolchain than the driver.\n"
            f"  driver built for {built_for}\n"
            f"  building with    {actual}\n"
            f"  rebuild for this toolchain:  cargo dbgvis setup\n"
            f"  or build with the driver's:  cargo +{toolchain} build")
...
# The exact toolchain the driver was compiled for, recorded next to it by
# `cargo dbgvis setup` (or `cargo xtask driver`). The driver is ABI-bound to it.
TOOLCHAIN_RECORD = DRIVER.parent / "toolchain"
...
def main():
    ...
    if not TOOLCHAIN_RECORD.exists():
        sys.exit("driver toolchain unknown: run `cargo dbgvis setup`"
                 " (or `cargo xtask driver` in the dbgvis repo)")
    built_for = TOOLCHAIN_RECORD.read_text().strip()
    if actual != built_for:
        sys.exit(_toolchain_mismatch(actual, built_for))
